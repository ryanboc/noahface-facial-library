...
<div class="container mx-auto px-4 py-8 max-w-[1500px]">
    ...
    <div class="grid xl:grid-cols-[1fr_330px] gap-6">
        ...
        <aside class="bg-white border rounded-xl shadow-sm p-5 h-fit"><h2 class="font-bold text-lg">Add a shift</h2><p class="text-sm text-gray-500 mb-5">Approved leave is checked automatically.</p><form method="POST" action="{{ route('roster.store') }}" class="space-y-4">@csrf
            <div><label class="text-sm font-medium">Employee</label><select name="employee_id" required class="mt-1 w-full border rounded-lg px-3 py-2.5"><option value="">Select employee</option>@foreach($employees as $employee)<option value="{{ $employee->id }}" @selected(old('employee_id') == $employee->id)>{{ $employee->name }}</option>@endforeach</select></div>
            ...
            <div><label class="text-sm font-medium">Notes</label><textarea name="notes" rows="2" class="mt-1 w-full border rounded-lg px-3 py-2.5">{{ old('notes') }}</textarea></div><button class="w-full bg-blue-600 hover:bg-blue-700 text-white rounded-lg py-2.5 font-semibold">Add shift</button>
        </form></aside>
    </div>
</div>
@endsection
